...
Original file is located at
    https://colab.research.google.com/drive/1_E_4MIMDLo8FE_uqlZiYcvipYgAQQ-nl
"""
...
def calculate_historical_momentum(tickers, category_name):
    ...
    # 依交易日計算報酬率 (1個月約21個交易日)
    m3 = prices.pct_change(periods=63)
    m6 = prices.pct_change(periods=126)
    m12 = prices.pct_change(periods=252)
...
def main():
    ...
    for cat_key, tickers in categories.items():
        ...
        category_result = calculate_historical_momentum(unique_tickers, cat_key)

        # 重組資料結構：以月份為主鍵，再分子類別
        for month, top3_list in category_result.items():
            if month not in final_json_data:
                final_json_data[month] = {}
            final_json_data[month][cat_key] = top3_list
    ...
    # 預覽最近一個月的結果
    latest_month = sorted(final_json_data.keys())[-1]
    print(f"\n[預覽] 最新月份 ({latest_month}) 前三名:")
    print(json.dumps(final_json_data[latest_month], indent=2, ensure_ascii=False))
